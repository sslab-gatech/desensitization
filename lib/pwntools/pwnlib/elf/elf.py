...
class ELF(ELFFile):
    ...
    def __init__(self, path, checksec=True):
        ...
        #: :class:`int`: Capsize of the ELF file per architecture
        self.capsz = 4 if self.arch == "i386" else 8
    ...
    def vaddr_to_offset(self, address):
        ...
        for interval in self.memory[address]:
            ...
            # return none if address does not fall into any segment mapped from program
            # headers in elf/core dump file, e.g., p_filesz = 0x0 or p_filesz < p_memsz
            if segment != "\x00":
    ...
    def write(self, address, data):
        ...
        if offset is not None:
            # NOTE: when offset is valid, we can overflow the segment bounds!
            length = len(data)
    ...
    def save(self, path=None, sparse=False):
        ...
        if path is None:
            path = self.path
        misc.write(path, self.data, sparse)
