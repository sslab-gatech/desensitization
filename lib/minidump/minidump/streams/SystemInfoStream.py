...
class MINIDUMP_SYSTEM_INFO:
    ...
    def parse(buff):
        ...
        msi.ProcessorArchitecture = unpack(buff.read(2))
        msi.ProcessorLevel = unpack(buff.read(2))
        msi.ProcessorRevision = unpack(buff.read(2))
        msi.NumberOfProcessors = unpack(buff.read(1))
        msi.ProductType = unpack(buff.read(1))
        msi.MajorVersion = unpack(buff.read(4))
        msi.MinorVersion = unpack(buff.read(4))
        msi.BuildNumber = unpack(buff.read(4))
        msi.PlatformId = unpack(buff.read(4))
        msi.CSDVersionRva = unpack(buff.read(4))
        msi.SuiteMask = unpack(buff.read(2))
        msi.Reserved2 = unpack(buff.read(2))
        if msi.ProcessorArchitecture == PROCESSOR_ARCHITECTURE.INTEL:
            for i in range(3):
                msi.VendorId.append(unpack(buff.read(4)))
            msi.VersionInformation = unpack(buff.read(4))
            msi.FeatureInformation = unpack(buff.read(4))
            msi.AMDExtendedCpuFeatures = unpack(buff.read(4))
        else:
            for i in range(2):
                msi.ProcessorFeatures.append(unpack(buff.read(8)))
        return msi
